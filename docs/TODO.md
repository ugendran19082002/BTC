...
- [x] **`monitorOn` is honoured (26 Sep).** The strategy form offers **On LTP / On candle close**,
      ...
      *finished* minute when `close` is asked for -- a wick through the level is not an exit, and a
      candle request that fails does nothing rather than falling back to the touch. The resting
      stop at Delta still triggers on the mark either way: it is there so the stop works when this
      process does not, and removing it would trade a slower exit for no exit at all during an
      outage. The form's hint says exactly that.
