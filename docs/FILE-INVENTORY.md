...
| `overview/DecisionPanels.tsx` | 330 | The compact option chain (near ATM / all / walls / recommended, quotes or greeks), the selected strike (greeks with their dollar meaning on hover; metrics / probability / payoff), the multi-timeframe table, the entry checklist by section with short gate labels and the PASS / FAIL count. |
| `overview/DecisionCards.tsx` | 220 | The four strategies side by side — SELL CE, SELL PE (direction → strike safety → premium quality → OI → risk → final), BOTH SIDES (each side safe, combined greeks, the final gate, ALLOWED / NOT ALLOWED), NO TRADE (confidence, the top five reasons, next recheck, what can change). |
| `overview/parts.tsx` | 104 | Panel, row, tag, fold, probability bar, number formats, `useWidth`. |
| `overview/TraderPanels.tsx` | 230 | The early-warning lamps (NORMAL / WATCH / TRIGGERED a trigger); Horizon / MTF (above / inside / below the band by horizon, with the MTF table and the board read); the what-changed table over 1m…12h with the premium's momentum from the same request; the strike finder (desk picks or filters; the filters also drive the SELL cards) with Sell. |
