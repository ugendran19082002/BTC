...
|---|---:|---|
| `engine.ts` | 1,047 | **The thing that actually trades.** Owns no clock and starts no timers: time arrives as `now()` and work arrives as `poll()`. Places entries, walks a resting limit toward the bid (`chasePrice`), reconciles protection against the book, watches the stop against the mark (`stopIfReached`) while the target only ever rests as a limit, closes, and serialises everything per trade through a promise-chain mutex so two screen actions can never interleave. |
...
| File | Lines | What it is for |
|---|---:|---|
| `lib/overview.ts` | 1100 | The arithmetic: IV vs RV, skew, expected move, premium analysis, odds, payoff, margin and fees, the sides assessed gate by gate, the side selector, both-sides safety, the risk engine, shocks, the 24-gate readiness, the position state ladder, horizons, the early warning, the strike finder. Pure; 50 tests. |
...
| `overview/MarketPanels.tsx` | 480 | KPI strip (spot, perp, funding, OI, volume, IV vs RV, PCR, regime, the CE / PE option bias), price action (trend, swings with BOS / CHOCH, RSI, MACD, VWAP, ATR, S/R distance), key levels as a table with ÷ ATR, volatility, the perp's trade flow and book, the options' own flow CE / PE with CVD and the board's lean, IV term structure with its history, skew with its percentile. Charts draw in pixels at their box width (`useWidth`). |
| `overview/DecisionPanels.tsx` | 280 | The compact option chain (near ATM / all / walls / recommended, quotes or greeks), the selected strike (greeks with their dollar meaning on hover; metrics / probability / payoff), the multi-timeframe table. |
| `overview/DecisionCards.tsx` | 120 | SELL CE beside SELL PE: direction → strike safety (P(OTM), touch, breach, distance) → premium quality → OI and the wall → risk (gamma, tail, liquidity, execution, margin) → the last word (PREFERRED / WATCH / NOT PREFERRED / NOT ALLOWED). The desk's answer is the panel's tag. |
| `overview/parts.tsx` | 104 | Panel, row, tag, fold, probability bar, number formats, `useWidth`. |
| `overview/TraderPanels.tsx` | 380 | The early-warning lamps (NORMAL / WATCH / TRIGGERED a trigger); Horizon / MTF (above / inside / below the band by horizon, with the MTF table and the board read); What changed — CE / PE / BOARD tabs, the premium read the seller's way, OI, IV, the model's P(OTM) / touch / distance then → now, one word a window (BETTER / NEUTRAL / WORSE), the last hour's state, and a since-entry row; the movement-type table (long buildup / short covering / short buildup / long unwinding by window, with volume strength, the tape's confirmation, the horizon's measured odds and implied move); the strike finder (desk picks or filters; the filters also drive the SELL cards) with Sell. |
| `overview/ScreenBar.tsx` | 95 | The bar above the screen: brand, IST clock, LIVE / EXPIRING / EXPIRED, the freshness pill (market · chain · OI · model age), the expiry list, the contract's day (entry now · window · expiry · time left), mode / refresh controls, a glossary. |
| `overview/RiskPanels.tsx` | 150 | The sell-side risk engine (IV − RV, tail, θ/γ, shocks, margin yield, hedge and protection distance) with the option-premium-decay chart and its 50 / 80 / 95 % milestones; the Scenario panel (BTC ±100/250/500, IV ±1/+2). |
| `lib/screen-config.ts` | 64 | The desk's fixed configuration (horizon, entry window, strictness, risk mode, side mode, execution, freshness, size) and the thresholds it sets. No UI changes it. |
...
## `app/server/test` — 16 files, 3,700 lines, 695 tests
...
## `app/web/src` tests — 26 files, 495 tests
