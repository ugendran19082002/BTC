...
| `overview/TraderPanels.tsx` | 300 | The early-warning lamps (NORMAL / WATCH / TRIGGERED a trigger); Horizon / MTF (above / inside / below the band by horizon, with the MTF table and the board read); What changed — CE / PE / BOARD tabs, the premium read the seller's way, OI, IV, the model's P(OTM) / touch / distance then → now, one word a window (BETTER / NEUTRAL / WORSE), the last hour's state, and a since-entry row; the strike finder (desk picks or filters; the filters also drive the SELL cards) with Sell. |
