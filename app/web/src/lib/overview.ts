...
export type NoTradeCard = {
  confidence: 'High' | 'Medium' | 'Low';
  /** The top reasons, failing gates across both sides and the desk's own checks, five at most. */
  reasons: string[];
  /** When to look again, IST "HH:MM": the entry window when it has not opened, the next five-minute mark otherwise. */
  recheckIst: string;
  /** What would have to change, one line per kind of failure. */
  waitFor: string[];
};

const IST_HM_FMT = new Intl.DateTimeFormat('en-GB', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: false });

/** What a failing gate is waiting on, in the trader's words. */
function waitingOn(name: string): string | null {
  const n = name.toLowerCase();
  if (n.includes('mtf') || n.includes('consensus') || n.includes('horizon')) return 'MTF alignment';
  if (n.includes('direction') || n.includes('regime')) return 'Direction to confirm';
  if (n.includes('liquidity') || n.includes('execution') || n.includes('spread') || n.includes('slippage')) return 'Spread improvement';
  if (n.includes('iv')) return 'IV expansion';
  if (n.includes('wall')) return 'OI wall movement';
  if (n.includes('pot') || n.includes('touch') || n.includes('distance') || n.includes('gamma')) return 'A further strike, or a calmer tape';
  if (n.includes('tail') || n.includes('margin') || n.includes('size') || n.includes('risk')) return 'Risk within the desk\'s limits';
  if (n.includes('window') || n.includes('entry')) return 'The entry window';
  if (n.includes('fresh') || n.includes('data')) return 'Fresh data';
  return null;
}

/**
 * NO TRADE as a strategy of its own, not an empty state: why, how sure, when
 * to look again, and what would have to change. Reasons are the failing
 * gates of both sides (named for the side), then the desk's own failing
 * checks; confidence is how many hard gates stand in the way.
 */
export function noTradeCard(sides: readonly SideAssessment[], gates: readonly Gate[], nowMs: number, entryIst: string): NoTradeCard {
  const sideFails = sides.flatMap((s) => (s.gates ?? []).filter((g) => g.ok === false).map((g) => ({ text: `${s.side} ${g.name}: ${g.text}`, name: g.name })));
  const deskFails = gates.filter((g) => g.ok === false).map((g) => ({ text: g.text, name: `${g.key} ${g.text}` }));
  const seen = new Set<string>();
  const all = [...sideFails, ...deskFails].filter((r) => (seen.has(r.text) ? false : (seen.add(r.text), true)));
  const reasons = all.slice(0, 5).map((r) => r.text);
  // Every side blocked by three or more gates is a clear no; one gate on one side is a near miss.
  const worst = Math.min(...sides.map((s) => (s.gates ?? []).filter((g) => g.ok === false).length), Number.POSITIVE_INFINITY);
  const confidence: NoTradeCard['confidence'] = !Number.isFinite(worst) || worst >= 3 ? 'High' : worst >= 2 ? 'Medium' : 'Low';
  const waitFor = [...new Set(all.map((r) => waitingOn(r.name)).filter((x): x is string => x !== null))];
  // The window when it is ahead today; otherwise the next five-minute mark.
  const windowMs = entryTodayMsOf(entryIst, nowMs);
  const recheckMs = windowMs !== null && windowMs > nowMs ? windowMs : Math.ceil((nowMs + 1) / 300_000) * 300_000;
  return { confidence, reasons, recheckIst: IST_HM_FMT.format(new Date(recheckMs)), waitFor };
}

/** The IST entry time as today's epoch ms. IST is UTC+5:30 with no daylight saving. */
function entryTodayMsOf(entryIst: string, nowMs: number): number | null {
