...
/**
 * The Overview: one screen for a short-premium decision, laid out after the
 * two reference designs in docs/image1.png and docs/image2.png.
 *
 *   KPIs across the top
 ...
 *
 * Every figure is read from the chain response or computed from it
 * (lib/overview.ts); what the desk does not capture is shown as not captured.
 * Nothing here places an order: the button opens the same ticket as the Live
 * screen, and the server runs every gate again.
 */
export function Overview({ data, trade, expiries, onExpiry, onSell, contracts, chart }: {
  data: ChainResponse;
  ...
  // The strike under inspection: the desk's own pick until someone clicks another.
