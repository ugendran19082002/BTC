...
test('[critical] a write is in the database before set() resolves', async () => {
  ...
  const row = await one<{ value: string }>('SELECT value FROM trading.settings WHERE key = $1', ['mode']);
...
test('overwriting keeps one row per key', async () => {
  ...
  const n = await one<{ n: string }>('SELECT COUNT(*)::text AS n FROM trading.settings WHERE key = $1', ['max_short_contracts']);
  assert.equal(n?.n, '1');
});
